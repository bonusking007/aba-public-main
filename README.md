...
getgenv().AltAccounts  = {"Anri8420", "yui_stream", "Sui_5096", "hisako619", "Takumi_7454", "Song7099", "taichi_saki", "Ze_Rurika", "Musatvizzi3621", "abafarmer96877567", "abafarmer912747567", "RicefarmerGrand1893", "grandfarmer357215", "Minesonos8632"}
-- ==================
...
-- ALT: ตรวจว่ามี main ในเซิฟ ถ้าไม่เจอใน 5 นาทีให้ hop
task.spawn(function()
	...
	-- ตรวจแรก: รอ 5 นาที ถ้าไม่เจอ main เลยให้ hop
	local found = false
	for _ = 1, 30 do -- loop 30 ครั้ง x 10 วิ = 5 นาที
		if checkMain() then found = true break end
		task.wait(10)
	end
	if not found then
		warn("[WWHub] Main not found after 5min — hopping...")
		pcall(function() game:GetService("TeleportService"):TeleportToRandomPlace(game.PlaceId) end)
		return
	end
	-- หลังจากนั้น loop check เรื่อยๆ ถ้า main หายไปแล้วไม่กลับมาใน 5 นาทีก็ hop
	while true do
		task.wait(10)
		if not getgenv().alt then break end
		if not checkMain() then
			warn("[WWHub] Main left server — waiting 5min before hop...")
			local came_back = false
			for _ = 1, 30 do -- 30 x 10 วิ = 5 นาที
				task.wait(10)
...
startFarm = function(mode)
	...
	loopMain = false
	makeBase()
	local function selectCharPlay()
		if mode == "alt" then
			-- alt ใช้ Random
			local bp = LP:FindFirstChild("Backpack")
			local inp = (bp and bp:FindFirstChild("Input"))
				or (getChar() and getChar():FindFirstChild("Input"))
				or LP:FindFirstChild("Input")
			if inp then pcall(function() inp:FireServer("CharacterButton", "Random") end) end
		else
...
gui.DisplayOrder  = 0  -- ไม่บัง BillboardGui ชื่อ player
...
-- Blocked mode check (ลด frequency เมื่อ idle)
task.spawn(function()
	while gui.Parent do
...
-- Garbage collect ทุก 60 วิ ป้องกัน memory leak
task.spawn(function()
	while gui and gui.Parent do
		task.wait(60)
